...
public class ContractValidationService implements ContractValidationUseCase {
    ...
    @Override
    public Map<Contract, List<Contract>> overlap(Collection<Contract> contractsToBeVerified, Long employeeId) {
        log.debug("Testing for overlapping contracts for employee [{}]", employeeId);
        Map<Contract, List<Contract>> overlappedResult = new HashMap<>();
        ...
        contractsToBeVerified.forEach(subjectContract -> {
            ...
            if (!overlappedFound.isEmpty()) {
                ...
                overlappedResult.put(subjectContract, overlappedFound);
            }
        });
    ...
    @Override
    public Map<Contract, List<Contract>> overlap(Collection<Contract> contracts) {
        log.debug("Starting comparison of overlapping contracts [{}].", contracts);
        List<Contract> givenContracts = new ArrayList<>(contracts);
        Map<Contract, List<Contract>> resultOverlapped = new HashMap<>();
        ...
        for (int i = 0; i < givenContracts.size(); i++) {
            ...
            for (int j = i + 1; j < givenContracts.size(); j++) {
                ...
                if (isOverlapped(contractA, contractB)) {
                    ...
                    resultOverlapped.computeIfAbsent(contractA, v -> new ArrayList<>()).add(contractB);
                    resultOverlapped.computeIfAbsent(contractB, v -> new ArrayList<>()).add(contractA);
                }
            }
        }
